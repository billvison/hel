...
public class ByteUtilTest {
    ...
    @Test
    public void long8ToBytes8Test()
    {
        for(int i=0;i<10000;i++){
            Random random = new Random();
            long number = random.nextLong();
            long resumeNumber = ByteUtil.byte8ToLong8(ByteUtil.long8ToByte8(number));
            //校验互转
            Assert.assertEquals(number,resumeNumber);
            //校验long转8字节大端数组
            Assert.assertArrayEquals(long8ToBytes8(number),ByteUtil.long8ToByte8(number));
        }

    }

    @Test
    public void bytes8ToLong8Test()
    {
        Random random = new Random();
        byte[] byte8 = new byte[8];
        for(int i=0;i<10000;i++){
            random.nextBytes(byte8);
            byte[] resumeByte8 = ByteUtil.long8ToByte8(ByteUtil.byte8ToLong8(byte8));
            //校验互转
            Assert.assertArrayEquals(byte8,resumeByte8);
            //校验8字节大端数组转long
            Assert.assertEquals(bytes8ToLong8(byte8),ByteUtil.byte8ToLong8(resumeByte8));
        }
    }
     ...
     */
    private static byte[] long8ToBytes8(long value) {
     ...
     */
    private static long bytes8ToLong8(byte[] bytes) {

 ...
 * ，后续业务需要使用区块哈希，是直接使用区块对象中区块哈希这个属性值
...
public class WritePropertyTool {
     ...
     */
    public static boolean checkBlockWriteProperties(Block previousBlock, Block block) {
        //校验写入的区块前区块哈希是否与计算得出一致
        if(!checkBlockWritePropertyPreviousBlockHash(previousBlock,block)){
            LogUtil.debug("区块写入的前区块哈希出错。");
            return false;
        }
        //校验写入的区块高度是否与计算得出一致
        if(!checkBlockWritePropertyBlockHeight(previousBlock,block)){
     ...
     * 校验写入的区块前区块哈希是否与计算得出一致
     */
    public static boolean checkBlockWritePropertyPreviousBlockHash(Block previousBlock, Block currentBlock) {
    ...
    /**
     * 校验区块中写入的区块高度属性是否与计算得出一致
     */
    public static boolean checkBlockWritePropertyBlockHeight(Block previousBlock, Block currentBlock) {
        if(previousBlock == null){
